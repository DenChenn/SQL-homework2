...
SELECT
    leagueName AS name,
    SUM(win_valid)/SUM(denominator) AS prob
FROM (
    SELECT
        leagueName,
        IF(avg_home_height > 180 AND avg_away_height > 180, 2, IF(avg_home_height > 180 OR avg_away_height, 1, 0)) AS denominator,
        IF(hts > aws AND avg_home_height > 180, 1, IF(hts < aws AND avg_away_height > 180, 1, 0)) AS win_valid
    FROM (
        SELECT 
